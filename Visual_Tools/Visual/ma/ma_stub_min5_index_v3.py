...
def fun(stocks):
    ...
    #version6 950之前五根k带ma5,10,20,60,120
    #version5 12根K
    #version4 5根K 无长均线
    ...
    table_name='A_Kline_Hybrid'+'_version1'
    ...
    for sc in stocks:
        ...
        data_day = KlineData.read_data(code=sc, start_date=Calendar.calc(start_time, -203)['date'], end_date=end_time,
                                   kline=kline2, timemerge=True)
        # data_day=data_day.sort_values(by=['date'],ascending=True)
        if len(data) <= 200 and len(data_day)<=200:
            continue
        ...
        data_day=CalMa.cal_ma_stub(data=data_day,interval=interval,detal=detal)
        ...
        while end_time > start_time:
            ...
            data_day=data_day[data_day.date<=date2]
            data1 = data_day.tail(length)



            # data0 = CalMacd.data_normalization(data0,pixel=pixel)
            data0=CalMa.data_stub_normalization2(data0,pixel=pixel)
            data1=CalMa.data_stub_normalization2(data1,pixel=pixel)
            ...
            try:
                visualization.draw_ma_stub_line3(data=data0,data_day=data1, profits=profit,
                                                date=date1, code=sc,
