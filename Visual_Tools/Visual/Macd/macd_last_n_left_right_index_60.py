...
def fun(stocks):
    ...
    for sc in stocks:
        ...
        d = data.date.iloc[-1]
        data = data[data.date <= dt.datetime(d.year, d.month, d.day, 10, 30)]
        print(sc)
        while end_time > start_time:
            ...
            if 'min' in kline:
                if 'A' in table_name:
                    if date1.hour != 10 and date1.minute != 30:
                        data = data[data.date <= dt.datetime(date2.year, date2.month, date2.day, 10, 30)]
                        break
