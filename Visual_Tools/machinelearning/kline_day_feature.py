...
def deal_data(code):
    data = KlineData.read_data(code=code, start_date=dt.datetime(2015, 1, 1), end_date=dt.datetime(2018, 9, 17),
                               kline='kline_day', timemerge=True)
    ...
    if len(data):
        ...
        jq_name=[ 'capitalization', 'circulating_cap', 'circulating_market_cap', 'market_cap', 'pb_ratio', 'pcf_ratio', 'pe_ratio', 'pe_ratio_lyr', 'ps_ratio', 'turnover_ratio']
        for i in jq_name:
            data[i]=0
        ...
        BaseModel('features_kline_day').insert_batch(data.to_dict(orient='records'))
...
def similarity():
    ...
    x_columns = [x for x in index_features.columns if
                 x in ["ma5_angle", "ma10_angle", "ma20_angle", "close_open", "high_low", 'high_close', 'Close_dt_ma5',
                       'Close_dt_ma10',
                       "Close_dt_ma20"]]  # "ma10_angle", "ma20_angle","ma30_angle", "ma60_angle",'close_ma5','open_ma5','high_open','high_low','high_close','close_open'
