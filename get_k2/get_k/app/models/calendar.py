...
class Calendar(BaseModel):
    ...
    # date = columns.Integer(index=True)
    # num = columns.Integer(index=True)
    ...
    @classmethod
    def in_business(cls, dt, hard=True, day=False):
        if day:
            if len(cls().query(date=dt)):
                return True
        else:
            # if cls().query(date=) is not None:
    ...
    @classmethod
    def trans_data(cls, source_data, length):
        _ = list()
    ...
    def calc(cls, date, offset):
        if isinstance(offset, datetime):
            ...
            if offset > 0:
                ...
                sd = cls.recent(date)
            td = Calendar().query(num=sd['num'] + offset)
            return td[0]
