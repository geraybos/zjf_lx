...
def angel(data):
    ...
    for i in range(len(data) - 1):
        if data[i + 1] == 0:
            ...
            result.append(math.atan(((data[i] / data[i + 1] - 1)) * 100) * 180 / 3.1415926)
    result.append(0)
    return result
...
def n_KDJ(data, n=9, m=3):
    ...
    data['K_down_cross_20'] = (data['K'] < 25)
    data['D_down_cross_20'] = (data['D'] < 25) & (data['D_z'] > data['D'])

    data = data[::-1]
    data['kdj_60_max'] = data['J'].rolling(window=60).max()
    data['kdj_30_max'] = data['J'].rolling(window=30).max()
    data['kdj_10_max'] = data['J'].rolling(window=10).max()
    data['kdj_5_max'] = data['J'].rolling(window=5).max()
    data['kdj_60_min'] = data['J'].rolling(window=60).min()
    data['kdj_30_min'] = data['J'].rolling(window=30).min()
    data['kdj_10_min'] = data['J'].rolling(window=10).min()
    data['kdj_5_min'] = data['J'].rolling(window=5).min()
    data['K_down_cross_20_count'] = data['K_down_cross_20'].rolling(window=15).sum()
    data['D_down_cross_20_count'] = data['D_down_cross_20'].rolling(window=15).sum()
    data['K_down_cross_20_count1'] = data['K_down_cross_20'].rolling(window=5).sum()
    data['D_down_cross_20_count1'] = data['D_down_cross_20'].rolling(window=5).sum()
    data = data[::-1]
...
def n_MA(data):
    ...
    data = data.iloc[::-1]
    data['close_up_ma20_count'] = data.close_up_ma20.rolling(window=15).sum()
    data = data.iloc[::-1]
    # k金叉均线数量
...
def MA(data):
    ...
    data['ma30_z'] = last_n(data, 1, 'ma30')
    data['ma60_z'] = last_n(data, 1, 'ma60')
    data['close_z'] = last_n(data, 1, 'close')
...
def MA_s(data):
    data = data[::-1]
    data['last_5'] = last_n(data, 5, 'close')
    data['last_10'] = last_n(data, 10, 'close')
    data['last_20'] = last_n(data, 20, 'close')
    data['last_30'] = last_n(data, 30, 'close')
    data['last_60'] = last_n(data, 60, 'close')
    data['ma5'] = data['close'].rolling(window=5).mean()
    data['ma10'] = data['close'].rolling(window=10).mean()
    data['ma20'] = data['close'].rolling(window=20).mean()
    data['ma30'] = data['close'].rolling(window=30).mean()
    ...
    data = data[::-1]
    data['close_z'] = last_n(data, 1, 'close')
    data['change_r'] = data['close'] / data['close_z']
    data['volume_z'] = last_n(data, 1, 'volume')
    data['volume_r'] = data['volume'] / data['volume_z']
